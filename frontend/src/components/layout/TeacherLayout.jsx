...
import {
  ...
  CalendarCheck, Megaphone, LogOut, Menu, X,
  Bell, ChevronRight
} from "lucide-react";

const NAV = [
  { to: "/teacher",            icon: LayoutDashboard, label: "Dashboard",  end: true },
  { to: "/teacher/results",    icon: ClipboardList,   label: "My Results" },
  { to: "/teacher/attendance", icon: CalendarCheck,   label: "Attendance" },
  { to: "/teacher/comms",      icon: Megaphone,       label: "Notices" },
];
    ...
    <div className="flex flex-col h-full bg-white border-r border-gray-100">
      <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100">
        ...
        <div className="min-w-0 flex-1">
          ...
          <span className="text-gray-400 text-xs">Teacher Portal</span>
        </div>
        {onClose && (
          ...
          <NavLink key={to} to={to} end={end} onClick={onClose}>
            {({ isActive }) => (
              <div className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium
                ...
                ${isActive ? "bg-emerald-600 text-white shadow-sm" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"}`}>
                <Icon className={`flex-shrink-0 ${isActive ? "text-white" : "text-gray-400 group-hover:text-gray-600"}`}
                  style={{ width: "18px", height: "18px" }} />
                <span className="flex-1">{label}</span>
                <ChevronRight className={`w-3.5 h-3.5 ${isActive ? "text-emerald-200" : "text-gray-200"}`} />
              </div>
            )}
          </NavLink>
        ))}
