...
import {
  ...
  ClipboardList, CalendarCheck, Wallet, Megaphone,
  LogOut, Menu, X, Bell, Settings, ChevronRight, UserCheck
} from "lucide-react";
    ...
    <div className="flex flex-col h-full bg-white border-r border-gray-100">
      <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100">
        ...
        <div className="min-w-0 flex-1">
          <span className="font-bold text-gray-900 text-base leading-tight block">SmartSchool</span>
          ...
          <NavLink key={to} to={to} end={end} onClick={onClose}>
            {({ isActive }) => (
              <div className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium
                ...
                <ChevronRight className={`w-3.5 h-3.5 ${isActive ? "text-primary-200" : "text-gray-200 group-hover:text-gray-300"}`} />
              </div>
            )}
          </NavLink>
        ))}
